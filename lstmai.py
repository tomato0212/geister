...
class Geister():
    ...
    def Board_Recode(self, server_response) -> list:
        ...
        for i in range(16):
            ...
            elif koma_x == 8 and koma_y == 8:
                if i < 8:
                    self.nige_flag = 1
                else:
                    self.nigerare_flag = 1
    ...
    # 手を決める
    def ThinkMove(self, hist_board, now_board, hist_cnt) -> int:
        if hist_cnt > 1:
            ...
            if escape_flag == 1:
                ...
                self.pred_flag = -1
                self.change(hist_board, now_board)
                self.pred_col(hist_board, now_board)
                eva = self.pred_negamax(0, -INF - 1, INF + 1)
                return eva
        else:
            escape_flag = self.EscapeCommand()
            if escape_flag == 1:
    ...
    # 勝利条件を確認
    def Judge(self, server_response) -> str:
    ...
    # 推定して評価値を求め，手を決める
    def pred_negamax(self, depth, alpha, beta):
        player = depth % 2
        win_player = self.GetWinplayer(player)
        if win_player == 0:
            if player == 0:
        ...
        if depth == MAX_DEPTH: # 最大まで探索した時
            b_eva = self.pred_Evaluate(player)
            return int(b_eva)
        ...
        for i in range(movenum):
            ...
            self.Board_list = self.pred_move(fr_list[i], to_list[i])
            
            res = -self.pred_negamax(depth+1, -beta, -alpha)
            if alpha < res:
                alpha = res
                if depth == 0:
                    self.BestMove = self.MoveCommand(fr_list[i], to_list[i])
    ...
    # 評価関数 teban = 0 -> 自分 | 1 -> 相手 (推定つきVer)
    def pred_Evaluate(self, player) -> int:
        s0, s1 = 0, 0
        existB_cnt = self.BlueCount()
        my_goaldist, enemy_goaldist = self.Distance()
        s0 = WEIGHT_CNT * existB_cnt - WEIGHT_DIR * my_goaldist
    ...
    # 駒色推定
    def pred_col(self, hist_board, now_board):
        pp_board = lp.read_board(hist_board, now_board)
        pred_color = lp.color_pred(pp_board)
        self.pred_color = pred_color
    ## R -> 0, B -> 1 が返る

    # 動いた駒の位置を把握する
    def change(self, hist_board, now_board):
        his = list(itertools.chain.from_iterable(hist_board))
        nn = list(itertools.chain.from_iterable(now_board))
        for i in range(36):
            if nn[i] != his[i]:
                if nn[i] == "u":
                    self.moving_place = i
    ...
    # 駒を動かしたときの処理
    def pred_move(self, fr, to) -> list:
        if to == self.moving_place:
            if self.pred_color == 0:
                self.pred_flag = 9
            elif self.pred_color == 1:
                self.pred_flag = 1
        if self.Board_list[fr] == "R":
            bb = self.pred_moveR(fr, to)
            return bb
        elif self.Board_list[fr] == "B":
            bb = self.pred_moveB(fr, to)
            return bb
        elif self.Board_list[fr] == "u":
            bb = self.moveu(fr, to)
            return bb
    ...
    # 自身の赤駒を動かした場合
    def pred_moveR(self, fr, to) -> list:
        tmp = self.Board_list
        tmp[fr] = "*"
        if tmp[to] == "u":
            self.komatori = 1
    ...
    # 自身の青駒を動かした場合
    def pred_moveB(self, fr, to) -> list:
        tmp = self.Board_list
        tmp[fr] = "*"
        if tmp[to] == "u":
            self.komatori = 1
    ...
    def Game(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client:
            ...
            # 盤面を記録する行列を作成
            hist_board = [[["*" for i in range(6)] for j in range(6)] for k in range(350)]
            if self.port == 10000:
                hist_cnt = 0
            if self.port == 10001:
                hist_cnt = 1

            moved_color_list = ["*" for i in range(350)]
            ...
            while server_response.startswith("MOV?") == True:
                ...
                for i in range(6):
                    for j in range(6):
                        hist_board[hist_cnt][i][j] = Board_Info[i][j]
                # ----------------------------------

                # 手を選択する処理 ↓ 
                res = self.ThinkMove(hist_board[hist_cnt-2], hist_board[hist_cnt], hist_cnt) 
                hist_cnt += 1
                # ----------------------------------
